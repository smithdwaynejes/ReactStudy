...
// data view
    dv.setFilter(item => {
      ...
      for (let key in item) {
        ...
        if (
          ...
          pass =
            pass &&
            String(item[key]).match(new RegExp(columnFilters[key], "ig"));
        } else if (
          key in columnFilters &&
          columnFilters[key].length &&
          key === "paymentDate"
    ...
    dv.getItemMetadata = index => {
      const row = this.props.slickDataViewgetItem(index);
...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    const changeFilter = _.debounce(value => {
      healthValue = value;
      this.props.slickDataViewrefresh();
    }, 500);
    ...
    grid.onSort.subscribe(function(e, args) {
      ...
      this.props.slickDataViewsort(comparer, args.sortAsc);
    });

    this.props.slickDataViewonRowCountChanged.subscribe(() => {
      grid.updateRowCount();
      grid.render();
    });
    ...
    grid.onCellChange.subscribe((e, { item }) => {
      this.props.slickDataViewupdateItem(item.id, item);
    });

    this.props.slickDataViewonRowsChanged.subscribe((e, { rows }) => {
  ...
  //   const idx = this.props.slickDataViewgetIdxById(id);
  //   if (idx === undefined || id !== item.id) {
  //     throw "Invalid or non-matching id";
  //   }
  //   this.props.slickDataViewgetItems()[idx] = item;
  //
  ...
  mutate = () => {
    ...
    this.props.slickDataViewgetItems().forEach(item => {
      ...
      if (item.currency === currency) {
        ...
        if (!hasUpdates) {
          hasUpdates = true;
          this.props.slickDataViewbeginUpdate();
        }
        this.props.slickDataViewupdateItem(item.id, item);
      }
    });

    hasUpdates && this.props.slickDataViewendUpdate();
...
const mapStateToProps = state => {
  return {
    slickColumns: state.slickData.slickColumns,
    slickData: state.slickData.slickData
  };
};
